...
def batch(model, image_folder, N, s=8, p1=0.2, save_maps=False, device=None):
    device = device or get_device()
    input_size = (224, 224)
    sfl_batch = SFL_batch(model, input_size, device=device)
    ...
    relevance_score_calculator = RelevanceScore(device=device)
    pixel_datasets = []
...
def main():
    ...
    results = batch(model, args.image_folder, args.N, args.s, args.p1, save_maps=True, device=device)
    if results:
